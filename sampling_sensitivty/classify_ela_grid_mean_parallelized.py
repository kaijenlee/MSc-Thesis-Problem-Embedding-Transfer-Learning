...
Variant: subsample both instances and runs for training, but aggregate the
sampled runs of each training instance into a SINGLE mean feature vector.

This differs from classify_ela_subsample_parallel.py only in how the
sampled training runs are turned into training rows:

  - subsample variant (original): each sampled run becomes its own training
    row, so an instance contributes n_runs_train rows.
  - mean variant (this file):     the sampled runs are averaged per feature,
    so an instance contributes exactly ONE row, regardless of n_runs_train.

At n_runs_train == 1 the two scripts are identical (the mean of a single run
is that run). The test side is unchanged: testing is still done on the median
feature vector (over all 30 runs), on all 30 individual runs, and via
majority vote.
...
For each configuration and each (n_instances_train, n_runs_train) pair:
  ...
  - From the 30 runs per training instance, subsample n_runs_train runs,
    then take the per-feature MEAN over those runs -> one row per instance
...
a common x-axis. It is unchanged from the original variant: averaging the
runs does not change how many function evaluations were spent generating
them, so the two scripts remain comparable on the same budget axis.

Usage:
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir --output-dir /out
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir --configs ilhs_50
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir --n-instances-train 1 5 10 20
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir --n-runs-train 1 3 5
  python classify_ela_subsample_mean_parallel.py /path/to/ela/pkl/dir --max-workers 8
"""
...
def run_classification(median_features, all_run_features, labels,
    ...
    MEAN VARIANT: the n_runs_train sampled runs of each training instance are
    averaged (per feature) into a single feature vector, so each training
    instance contributes exactly one row.  This is the only difference from
    the original per-run-row variant.
    ...
    for fold_idx, (test_idx, train_idx) in enumerate(skf.split(
        ...
        # Gather the sampled runs for each selected training instance:
        #   shape (n_train, n_runs_train, n_features)
        X_train_runs = all_run_features[selected_train_idx][row_idx,
                                                            sampled_runs, :]

        # --- MEAN AGGREGATION ---------------------------------------------
        # Average the sampled runs per feature so each instance contributes
        # ONE row, regardless of n_runs_train.  nanmean is used so that an
        # occasional NaN in one run does not wipe out the whole average; this
        # mirrors the NaN-tolerance used elsewhere in the pipeline (the test
        # median is also computed with nanmedian, and StandardScaler ignores
        # NaNs while nan_to_num cleans up any residual ones below).
        X_train = np.nanmean(X_train_runs, axis=1)   # (n_train, n_features)
        y_train = labels[selected_train_idx]         # (n_train,)
        # ------------------------------------------------------------------
        ...
        del X_train, X_train_runs, X_train_scaled
...
def _writer_loop(write_queue, output_file):
    ...
    with h5py.File(output_file, "a") as out:
        while True:
            ...
            sub_grp.attrs["train_run_aggregation"] = "mean"
...
def main(input_dir, output_dir=None, configs=None,
    ...
    output_file = Path(output_dir) / "ela_classification_subsample_mean.h5"
    ...
    print(f"ELA Classification Experiment "
          f"(parallel, subsampled instances & runs, MEAN over training runs)")
    ...
    print(f"Training run aggregation: MEAN (1 row per training instance)")
    ...
    print(f"             dimension, n_feval_train, train_run_aggregation,")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Classification experiment for ELA features "
                    "(parallel, subsampled instances and runs; training runs "
                    "averaged into a single mean feature vector per instance)."
    )
