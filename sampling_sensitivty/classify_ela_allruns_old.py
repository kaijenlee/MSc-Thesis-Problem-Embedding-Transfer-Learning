...
For each configuration:
  ...
  - Train on all 30 runs per instance (expanded training set)
  - Test on individual runs (30 predictions per test instance)
  - Record per-instance consistency and overall accuracy

Usage:
  python classify_ela_allruns_old.py /path/to/ela/pkl/dir
  python classify_ela_allruns_old.py /path/to/ela/pkl/dir --output-dir /path/to/output
  python classify_ela_allruns_old.py /path/to/ela/pkl/dir --configs ilhs_50 sobol_50
  python classify_ela_allruns_old.py /path/to/ela/pkl/dir --n-jobs -1
"""
...
def run_classification(median_features, all_run_features, labels, n_jobs=1):
    ...
    Train on all individual runs (expanded), test on individual runs
    and on median features.
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(
        ...
        X_train = all_run_features[train_idx].reshape(n_train * N_RUNS, N_FEATURES)
        y_train = np.repeat(labels[train_idx], N_RUNS)
        ...
        for i, test_row in enumerate(test_idx):
            # All 30 runs for this test instance at once
    ...
    all_true_labels = np.array(all_true_labels)              # (n_instances,)
    all_pred_median = np.array(all_pred_median)              # (n_instances,)
    all_pred_runs = np.array(all_pred_runs)                  # (n_instances, N_RUNS)

    # Majority vote per instance: most frequent prediction across 30 runs
    majority_vote = mode(all_pred_runs, axis=1, keepdims=False).mode  # (n_instances,)
...
def main(input_dir, output_dir=None, configs=None, n_jobs=1):
    ...
    print(f"ELA Classification Experiment (Train on All Runs)")
    ...
    with h5py.File(output_file, "a") as out:
        for config_key in config_keys:
            ...
            print(f"    Fold accuracies (median-test): {results['fold_accuracies_median']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Classification experiment for ELA features (train on all runs)."
    )
