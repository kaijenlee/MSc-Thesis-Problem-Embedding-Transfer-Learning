...
_RATIO_STRATEGY_ORDER = ["sobol", "ilhs", "lhs_random_cd", "lhs", "uniform",
                         "cma_random"]


def plot_ratio_w_distribution(ratios, dimensions=(2, 5, 10), sizes=None,
                              strategies=None, kind="box", logy=True,
                              clip_pct=(1, 99), show_median=True,
                              width_per=4.6, height_per=4.2, title=None):
    """Distribution of the noise ratio, per (strategy, sample size, dimension).

    The summary tables report only the median of rho_w over the roughly 24 x 45
    (function, feature) cells that contribute to each strategy and sample size.
    That conceals how much the ratio varies underneath: a median of 0.66 could
    describe a strategy that is uniformly two-thirds as variable as uniform
    sampling, or one that is far quieter on some features and no better on
    others. The distinction matters, because a claim that one strategy is more
    stable than another is much weaker if their distributions overlap.

    One panel per dimension, one box per (strategy, size). The dashed line at
    1.0 is uniform sampling, which is the baseline and therefore degenerate.

    The vertical axis is logarithmic by default, because the ratios are
    multiplicative: a strategy at 0.5 and one at 2.0 are equally far from the
    baseline in opposite directions, which a linear axis misrepresents. Extreme
    cells are trimmed by `clip_pct` for display only; they arise where uniform
    sampling itself produced almost no variation, so that the denominator of
    Equation (ratio_w) is near zero.
    """
    d = ratios[ratios["dimension"].isin(dimensions)]
    d = d[np.isfinite(d["ratio_w"]) & (d["ratio_w"] > 0)]
    ...
    strats = [s for s in _RATIO_STRATEGY_ORDER if s in set(d["strategy"])]
    ...
    fig, axes = plt.subplots(1, len(dims),
                             figsize=(width_per * len(dims), height_per),
                             squeeze=False, sharey=True)
    axf = axes[0]
    width = 0.8 / max(len(szs), 1)

    for ax, dim in zip(axf, dims):
        sub = d[d["dimension"] == dim]
        for si, sz in enumerate(szs):
            offs = (si - (len(szs) - 1) / 2) * width
            data, pos, cols = [], [], []
            for xi, st in enumerate(strats):
                v = sub.loc[(sub["strategy"] == st) & (sub["size"] == sz),
                "ratio_w"].to_numpy()
                if v.size:
                    data.append(v);
                    pos.append(xi + offs);
                    cols.append(color_of[st])
            if not data:
                continue
            if kind == "box":
                ...
                for p, c in zip(bp["boxes"], cols):
                    p.set_facecolor(c)
                    p.set_alpha(0.35 + 0.55 * si / max(len(szs) - 1, 1))
                    p.set_edgecolor(c)
            else:
                vp = ax.violinplot(data, positions=pos, widths=width * 0.85,
                                   showextrema=False, showmedians=True)
                for b, c in zip(vp["bodies"], cols):
                    b.set_facecolor(c)
                    b.set_alpha(0.35 + 0.55 * si / max(len(szs) - 1, 1))
            if show_median:
                for p, v in zip(pos, data):
                    ax.plot(p, np.median(v), marker="_", ms=width * 34,
        ...
        if logy:
            ax.set_yscale("log")
        if clip_pct:
            lo, hi = np.percentile(sub["ratio_w"], clip_pct)
            ax.set_ylim(lo / 1.3, hi * 1.3)
        ax.set_xticks(range(len(strats)))
        ax.set_xticklabels(strats, rotation=30, ha="right", fontsize=8.5)
        ax.set_xlim(-0.6, len(strats) - 0.4)
        ax.set_title(f"dimension {dim}", fontsize=11)
        ax.grid(True, axis="y", which="both", alpha=0.3)
    axf[0].set_ylabel("$\\rho_w$  (vs uniform sampling)")

    shade = [Patch(facecolor="grey",
                   alpha=0.35 + 0.55 * i / max(len(szs) - 1, 1),
                   label=f"{s}$\\times$d") for i, s in enumerate(szs)]
    fig.tight_layout(rect=[0, 0, 0.88, 0.92])
    fig.legend(handles=shade, loc="center left", bbox_to_anchor=(0.89, 0.5),
               title="sample size", frameon=False)
    fig.suptitle(title or "Distribution of the run-to-run noise ratio "
                          "across functions and features", fontsize=12)
    return fig, axes


def heatmap_ratio_w_by_feature(ratios, dimension, size, strategies=None,
                               features=None, sort_by=None, vmax=None,
                               annotate=False,
                               width=8.4, height_per_row=0.22, title=None):
    """Which FEATURES benefit from a low-discrepancy design, and which do not.

    The summary tables give one number per strategy, but that number averages
    over features whose behaviour differs by more than an order of magnitude.
    Aggregating by function instead gives a much narrower range, so the variation
    is a property of the features rather than of the landscapes; this figure
    therefore uses features as its axis.

    Rows are features, columns are sampling strategies, and each cell is the
    median rho_w over the 24 functions. The colour scale is centred on 1.0, the
    value taken by uniform sampling: blue marks a feature measured more stably
    than under uniform sampling, red one measured less stably. A row that is
    uniformly blue is a feature for which the choice of design matters; a row
    near white throughout is one for which it does not.

    `sort_by` names a strategy to order the rows by, which groups the features
    that design helps most at the top.
    """
    d = ratios[(ratios["dimension"] == dimension) & (ratios["size"] == size)]
    print(d["feature"].unique())
    d = d[np.isfinite(d["ratio_w"]) & (d["ratio_w"] > 0)]
    print(d["feature"].unique())
    if strategies is not None:
        d = d[d["strategy"].isin(strategies)]
    ...
    M = (d.pivot_table(index="feature", columns="strategy", values="ratio_w",
                       aggfunc="median"))
    strats = [s for s in _RATIO_STRATEGY_ORDER if s in M.columns]
    strats += [c for c in M.columns if c not in strats]
    M = M[strats]
    key = sort_by if (sort_by and sort_by in M.columns) else strats[0]
    M = M.loc[M[key].sort_values().index]

    A = M.to_numpy(dtype=float)
    if vmax is None:
        vmax = float(np.nanpercentile(A, 98))
    # symmetric in log space about 1.0, since the values are ratios
    lo = 1.0 / vmax

    fig, ax = plt.subplots(figsize=(width, max(3.0, height_per_row * len(M) + 1.8)))
    im = ax.imshow(np.log10(A), aspect="auto", cmap="RdBu_r",
                   vmin=np.log10(lo), vmax=np.log10(vmax),
                   interpolation="nearest")
    ax.set_xticks(range(len(strats)))
    ax.set_xticklabels(strats, rotation=30, ha="right", fontsize=9)
    ax.set_yticks(range(len(M)))
    ax.set_yticklabels(M.index, fontsize=6.5)
    ax.set_xticks(np.arange(-0.5, len(strats), 1), minor=True)
    ax.set_yticks(np.arange(-0.5, len(M), 1), minor=True)
    ax.grid(which="minor", color="#b0b0b0", lw=0.4, alpha=0.8)
        ...
        for i in range(A.shape[0]):
            for j in range(A.shape[1]):
                if np.isfinite(A[i, j]):
                    ax.text(j, i, f"{A[i, j]:.2f}", ha="center", va="center",
                            fontsize=5.5,
                            color="white" if abs(np.log10(A[i, j])) > 0.35
                            else "black")

    cb = fig.colorbar(im, ax=ax, fraction=0.035, pad=0.02)
    ticks = [lo, (1 + lo) / 2, 1.0, (1 + vmax) / 2, vmax]
    ticks = sorted({round(t, 2) for t in ticks if t > 0})
    cb.set_ticks(np.log10(ticks))
    cb.set_ticklabels([f"{t:g}" for t in ticks])
    cb.set_label("median $\\rho_w$ over functions  (1 = uniform sampling)",
                 fontsize=9)
    ax.set_title(title or f"Run-to-run noise ratio by feature  "
                          f"[dim {dimension}, $n = {size}\\times d$]"
                          # f"\nrows sorted by {key}"
                 , fontsize=11)
    fig.tight_layout()
    return fig, ax
...
def heatmap_ratio_w_by_function(ratios, dimension, size, strategies=None,
                                funcs=None, sort_by=None, vmax=None,
                                annotate=True, group_lines=True,
                                width=8.4, height_per_row=0.26, title=None):
    """The by-function counterpart of heatmap_ratio_w_by_feature.

    Rows are BBOB functions, columns are sampling strategies, and each cell is
    the median rho_w over the features. This is a sanity check on the claim that
    the variation in stability is a property of the features rather than of the
    landscapes: if that holds, this figure should be far flatter than the
    by-feature one, with the rows differing little from each other and from the
    strategy's overall median.

    A row that stands out would indicate a landscape on which the sampling
    design matters unusually much or unusually little, which would qualify the
    claim.
    """
    d = ratios[(ratios["dimension"] == dimension) & (ratios["size"] == size)]
    d = d[np.isfinite(d["ratio_w"]) & (d["ratio_w"] > 0)]
    ...
    M = d.pivot_table(index="func", columns="strategy", values="ratio_w",
                      aggfunc="median")
    strats = [s for s in _RATIO_STRATEGY_ORDER if s in M.columns]
    strats += [c for c in M.columns if c not in strats]
    M = M[strats]
    if sort_by and sort_by in M.columns:
        M = M.loc[M[sort_by].sort_values().index]
    else:
    ...
    ax.set_xticklabels(strats, rotation=30, ha="right", fontsize=9)
    ax.set_yticks(range(len(M)))
    ax.set_yticklabels([f"f{f}" for f in M.index], fontsize=7.5)
    ax.set_xticks(np.arange(-0.5, len(strats), 1), minor=True)
    ax.set_yticks(np.arange(-0.5, len(M), 1), minor=True)
    ax.grid(which="minor", color="#b0b0b0", lw=0.4, alpha=0.8)
    ax.tick_params(which="minor", length=0)
    ax.set_axisbelow(False)
    if group_lines and sort_by is None:
        pos = {f: i for i, f in enumerate(M.index)}
        for b in (5, 9, 14, 19):
            if b in pos:
                ax.axhline(pos[b] + 0.5, color="black", lw=1.0, alpha=0.6)
    if annotate:
        for i in range(A.shape[0]):
            for j in range(A.shape[1]):
                if np.isfinite(A[i, j]):
    ...
    cb = fig.colorbar(im, ax=ax, fraction=0.035, pad=0.02)
    ticks = sorted({round(t, 2) for t in
                    [lo, (1 + lo) / 2, 1.0, (1 + vmax) / 2, vmax] if t > 0})
    cb.set_ticks(np.log10(ticks));
    cb.set_ticklabels([f"{t:g}" for t in ticks])
    cb.set_label("median $\\rho_w$ over features  (1 = uniform sampling)",
                 fontsize=9)
    ax.set_title(title or f"Run-to-run noise ratio by function  "
                          f"[dim {dimension}, $n = {size}\\times d$]", fontsize=11)
    fig.tight_layout()
    return fig, ax